...
def rmse(img1, img2, dynamic_range=255):
    ...
    return np.sqrt(mse)
def dd(img1, img2, dynamic_range=255):
    """PSNR metric, img uint8 if 225; uint16 if 2047"""
    if not  img1.shape == img2.shape:
        raise ValueError('Input images must have the same dimensions.')
    img1_ = img1.astype(np.float64)
    img2_ = img2.astype(np.float64)
    mae = np.mean(np.abs(img1_ - img2_))
    return mae
...
def psnr_HSI(img1, img2, dynamic_range=255):
    """PSNR metric, img uint8 if 225; uint16 if 2047"""
    if not  img1.shape == img2.shape:
        raise ValueError('Input images must have the same dimensions.')
    img1_ = img1.astype(np.float64)
    img2_ = img2.astype(np.float64)
    c=np.zeros(img1.shape[2])
    for i in range(img1.shape[2]):
        ...
        if mse <= 1e-10:
            return np.inf
        c[i]=20 * np.log10(255 / (np.sqrt(mse) + np.finfo(np.float64).eps))
    return np.mean(c)
...
def rmse1(Fuse1,HRHSI):
    ...
    for j in range(Fuse1.shape[0]):
        be = np.mean(np.square(np.float64(np.uint8(np.round(Fuse1[j,...]*255)))-np.float64(np.uint8(np.round(HRHSI[j,...]*255)))))
